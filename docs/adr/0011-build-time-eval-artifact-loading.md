...
- **Build needs repo-relative access to `evals/`.** Works in `npm run build` from `frontend/` (walks up one level) and in `npm run dev`. **Does not work in the current Railway Dockerfile build context** — the frontend Dockerfile copies only `frontend/`, so `evals/` is not in the image. The loader returns null in that case and the dashboard renders a "no eval run found" placeholder. A follow-up PR will either widen the Docker context or expose eval data through the backend API; until then, the live deploy renders with the placeholder while local builds render the real numbers.
