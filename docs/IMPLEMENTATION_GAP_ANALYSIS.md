...
- **Eval harness upgrade — shipped (PR #27).** Item #14 in the table. Sliced-metric bug fixed (per-category support now reflects the slice, not the full dataset). New metric blocks: confusion pairs / F1 / category coverage, routing (auto-approved rate + auto-approved accuracy + review rate + zero-cost share + provider split + cost savings vs baseline), and calibration with separated `model_only` and `deterministic` blocks (ECE, MCE, high-confidence warning). New eval modes: `hybrid-rules-model` mirrors the production rules→model fallback, and `HybridMemoryRulesModelCategorizer` simulates correction memory from a deterministic train/test split with an explicit label-leakage guard. New comparison CLI `python -m ledgerlens.evals.compare` emits `evals/runs/YYYY-MM-DD-comparison.{json,md}`. The `/evals` frontend page surfaces the layered-pipeline summary, mode comparison table, routing tiles, calibration warning, and top confusions.

The priority table below is kept in its original form for traceability — items 1–10, 12, 13, and 14 are now done; item 11 and items 15 onward remain as the forward backlog.
