...
const PHOTO_CREDITS_COMPONENT = readFileSync(
  join(SRC, "components", "marketing", "PhotoCredits.tsx"),
  "utf-8",
);
const IMAGE_CREDITS_MODULE = readFileSync(
...
describe("homepage stock photography (prep — disabled by default)", () => {
  ...
  it("enables hero + trust + auto-shop + engineering; faq remains disabled", () => {
    const enabledMatches =
      HOMEPAGE_IMAGES_MODULE.match(/enabled:\s*true/g) ?? [];
    expect(enabledMatches.length).toBe(4);
